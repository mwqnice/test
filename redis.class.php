...
class FM_Redis
{
    ...
    private function __construct($config, $attr = array())
    {
        ...
        $connectConfig = $this->config[$config];
        ...
        $this->port = $connectConfig['port'] ? $connectConfig['port'] : 6379;
        $this->host = $connectConfig['host'];
        $this->redis->connect($this->host, $this->port, $this->attr['timeout']);

        if ($connectConfig['auth']) {
            $this->auth($connectConfig['auth']);
            $this->auth = $connectConfig['auth'];
        }
